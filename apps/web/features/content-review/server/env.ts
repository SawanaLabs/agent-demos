...
import { getContentReviewAppEnv } from "./env-source";
...
export type ContentReviewEnv = Record<string, string | undefined>;
...
export interface ContentReviewSetupState {
  config: Omit<ContentReviewConfig, "apiKey">;
  isReady: boolean;
  issues: string[];
  nodeVersion: string;
}
...
export function getContentReviewEnv(): ContentReviewEnv {
  return getContentReviewAppEnv();
}

function readRequiredEnv(
  env: ContentReviewEnv,
  name: keyof ContentReviewEnv
) {
...
function resolveContentReviewEnv(
...
export function getContentReviewConfig(
  env: ContentReviewEnv = getContentReviewEnv()
): ContentReviewConfig {
  assertSupportedNodeRuntime();
  const resolvedEnv = resolveContentReviewEnv(env);
...
export function getContentReviewSetupState(
  env: ContentReviewEnv = getContentReviewEnv()
): ContentReviewSetupState {
  const issues: string[] = [];
  const resolvedEnv = resolveContentReviewEnv(env);
...
export function createContentReviewGateway(
  env: ContentReviewEnv = getContentReviewEnv()
) {
  const { apiKey, baseURL } = getContentReviewConfig(env);
