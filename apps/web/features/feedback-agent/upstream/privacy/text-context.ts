...
type SensitiveTextState =
  | "none"
  | "sensitive"
  | "contains-sensitive-child"
  | "too-large";
...
export function hasSensitiveTextContext(
  node: Node,
  ...
  while (
    ...
    if (state === "contains-sensitive-child" || state === "too-large") {
      return false;
    }
...
function sensitiveTextState(
  ...
  if (text.length > MAX_GROUPED_TEXT_LENGTH) {
    ...
    const childContainsSensitiveText = Array.from(element.children).some(
      (child) => {
        const childState = sensitiveTextState(child, context);
        return (
          childState === "sensitive" ||
          childState === "contains-sensitive-child"
        );
      }
    );
    state = childContainsSensitiveText
      ? "contains-sensitive-child"
      : "sensitive";
  }
