...
const FIELD_SEMANTIC_KEYS = new Set([
  "type",
  "autocomplete",
  "name",
  "id",
...
const SENSITIVE_KEY_PATTERN =
  /(?:^|_)(?:password|passwd|passcode|card_(?:number|no)(?:_[0-9]+)?|credit_card|cc_(?:number|num|csc|cvv|cvc|exp|expiry)|cvv|cvc|csc|security_code|one_time_(?:code|password)|otp(?:_code)?|verification_code|auth(?:entication)?_token|access_token|refresh_token|id_token|token|api_key|client_secret|secret|authorization|cookie|set_cookie|credential|private_key|hidden_value|file_name|filename)(?:$|_(?:value|field|input|confirmation|confirm|header|data|pem)$)/;
...
export function filterSensitiveText(value: string): string {
  ...
  return filtered.replace(
    HTTP_URL_PATTERN,
    (candidate) => sanitizeUrl(candidate) || FILTERED_VALUE
  );
}
...
export function filterSensitiveValue<T>(value: T, key?: string): T {
  if (key && isSensitiveKey(key) && value != null) {
    return FILTERED_VALUE as T;
  }
  if (typeof value === "string") {
    return filterSensitiveText(value) as T;
  }
  if (Array.isArray(value)) {
    return value.map((item) => filterSensitiveValue(item)) as T;
  }
  if (value && typeof value === "object") {
    const sensitiveField = isSensitiveFieldAttributes(
    ...
    const filtered = Object.fromEntries(
      Object.entries(value).map(([nestedKey, item]) => {
        const filteredItem =
          sensitiveField &&
          VALUE_ATTRIBUTE_KEYS.has(normalizeIdentifier(nestedKey)) &&
          item != null
            ? FILTERED_VALUE
...
export function isSensitiveFieldAttributes(
  ...
  return Object.entries(attributes).some(([key, value]) => {
    ...
    if (!FIELD_SEMANTIC_KEYS.has(normalizedKey)) {
      return false;
    }
    if (normalizedKey === "type") {
      return SENSITIVE_INPUT_TYPES.has(normalizeIdentifier(value));
    }
    if (normalizedKey === "autocomplete") {
      return value
        .split(/\s+/)
        .some((token) =>
          SENSITIVE_AUTOCOMPLETE.has(normalizeIdentifier(token))
        );
    }
