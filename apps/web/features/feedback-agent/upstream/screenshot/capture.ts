...
let fontEmbedCssRequest: Promise<string | null> | null = null;

export function warmFontEmbedCss(): Promise<string | null> {
  if (fontEmbedCss !== undefined) {
    return Promise.resolve(fontEmbedCss);
  }
...
export async function captureBaseScreenshot(
  ...
  try {
    ...
    const [{ toJpeg }, fontEmbedCSS] = await Promise.all([
      loadHtmlToImage(),
      warmFontEmbedCss(),
    ]);
    if (signal?.aborted) {
    ...
    const dataUrl = await toJpeg(document.body, {
      ...
      filter: (node) => {
        if (
          ...
          (node.id === WIDGET_HOST_ID || node.closest("[data-feedback-ui]"))
        ) {
          return false;
        }
...
export async function bakeAnnotatedScreenshot(
  ...
  try {
    const screenshotAnnotations = annotations.map((annotation) =>
      translateAnnotationForScreenshot(annotation, base.pageScroll)
    );
    return await bakeScreenshot(
      base.dataUrl,
...
function translateAnnotationForScreenshot(
  ...
  return {
    ...
    drawPath:
      annotation.drawPath &&
      translatePathD(annotation.drawPath, offset.x, offset.y),
  };
}
 ...
 */
function screenshotOffset(
  annotation: Annotation,
  pageScroll: ScrollOffset
): ScrollOffset {
  if (
    annotation.captureOffsetX !== undefined &&
    annotation.captureOffsetY !== undefined
  ) {
...
function collectPrivacyRedactions(root: HTMLElement): Rect[] {
  ...
  for (const element of root.querySelectorAll("input, textarea, select")) {
    ...
    const hasSensitiveValue =
      isSensitiveField(element) || filterInputValue(value, element) !== value;
    if (!(isMasked || hasSensitiveValue)) {
      continue;
    }
    addRedaction(
      redactions,
  ...
  collectTextRedactions(root, redactions);
  return redactions;
}

function collectTextRedactions(root: HTMLElement, redactions: Rect[]): void {
  ...
  while (node) {
    ...
    const hasSensitiveText =
      filterSensitiveText(text) !== text ||
      hasSensitiveTextContext(node, textContext);
    if (
      parent &&
      !isWidgetOrBlocked(parent) &&
      (isMasked || hasSensitiveText)
    ) {
...
function isWidgetOrBlocked(element: Element): boolean {
  return (
    element.closest(`#${WIDGET_HOST_ID}`) !== null ||
    element.closest(`.${BLOCK_CLASS}`) !== null
  );
}
...
function drawStroke(
  ...
  for (const points of parsePathPolylines(drawPath)) {
    ...
    ctx.beginPath();
    ctx.moveTo(toCanvasX(transform, first[0]), toCanvasY(transform, first[1]));
    for (const point of rest) {
      ctx.lineTo(
        toCanvasX(transform, point[0]),
        toCanvasY(transform, point[1])
      );
    }
...
function capturePixelRatio(metrics: Rect): number {
  ...
  if (!Number.isFinite(total) || total <= MAX_CAPTURE_PIXELS) {
    return 1;
  }
