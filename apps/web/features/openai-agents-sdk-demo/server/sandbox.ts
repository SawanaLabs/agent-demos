...
function getOpenAiAgentsSdkDemoRepoRoot() {
  const cwd = process.cwd();
  const rootLikeCwd = resolve(cwd);
  const appLikeCwd = resolve(cwd, "../..");
  ...
  throw new Error(
    `Could not resolve the repository root for the OpenAI Agents SDK sandbox demo from cwd ${cwd}.`
  );
}

const sandboxRepoRoot = getOpenAiAgentsSdkDemoRepoRoot();
const sandboxMountedEntries = {
  docs: resolve(sandboxRepoRoot, "docs/frontend"),
