...
describe("vercel sandbox integration test command", () => {
  it("keeps real sandbox tests behind an explicit package script", async () => {
    ...
    expect(rootPackage.scripts["test:unit"]).toBe(
      "pnpm --dir apps/web test:unit && node --test scripts/registry-sync/registry-projection.test.mjs scripts/observability/keys.test.mjs scripts/observability/worktree-stack.test.mjs && pnpm registry:telemetry:check && pnpm registry:catalog:check"
    );
