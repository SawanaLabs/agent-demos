...
- Preserve Next.js route prefetching on demo entry links and button-driven demo navigation. Setting demo links to `prefetch={false}` makes dynamic routes wait for server rendering before navigation; production QA for loading fallbacks should use a production build because automatic `<Link>` prefetching is disabled in `next dev`.
...
- Do not render user-actionable chat, composer, attachment-preparation, API route, provider/model, tool/runtime, sandbox, or stream errors as workspace-level banners above the conversation. If the error would otherwise be a red banner inside the chat panel, render it as an assistant-side **Conversation Error Message** row near the current turn so the evaluator does not need to scroll back to discover the failure. Muted setup availability text can remain a non-error workspace state when the chat cannot be started yet.
- For route-backed chat demos that send only the last message, pass AI SDK `trigger` and `messageId` through `prepareSendMessagesRequest` in addition to the last `message`. The route handler should feed those fields into the **Route-backed Chat Replay Module** before model conversion.
- Treat `trigger: "regenerate-message"` as replay from a persisted user turn. The route handler should trim persisted messages after that user turn when an assistant/tool suffix exists, upsert the incoming user message once, clear stale stream state through the store path, and save the new assistant response only from the stream finish callback.
