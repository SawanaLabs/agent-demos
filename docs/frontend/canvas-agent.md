...
- The Agent defaults to execute mode but runs only on explicit generation requests. New canvas clears the graph and conversation. `readWorkflow`, `editWorkflow`, `runWorkflow`, and `arrangeCanvas` cover inspection, graph edits, execution, and browser-measured layout. Follow-up generation creates a connected node and runs that target, retaining earlier outputs. Layout requests apply after streaming finishes.
