...
- The Vercel-compatible FastAPI backend is deployed as the separate Vercel project `agent-demos-langgraph-agent-api`, with production alias `https://agent-demos-langgraph-agent-api.vercel.app`. Keep `apps/langgraph-agent-api/vercel.json` in place with `framework: "fastapi"`; without that preset, a CLI-created Vercel project can treat `app.py` as a static file and return 404 for `/threads`.
