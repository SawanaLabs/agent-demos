...
- When skills-agent setup is unavailable, disable every chat-turn entry path, including the composer, submit button, sample prompts, and retry. UI handlers should also guard against sending requests while setup is unavailable or a turn is already busy.
