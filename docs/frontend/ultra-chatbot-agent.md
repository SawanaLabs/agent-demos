...
- Add an artifact/document panel as the companion surface for generated or edited documents.
...
- The fifteenth implemented slice now promotes the document companion into an explicit artifact shell. `ui/use-ultra-chatbot-agent-artifact.ts` plus `ui/ultra-chatbot-agent-artifact-state.ts` own selected-document, refresh, and mode state; `ui/ultra-chatbot-agent-artifact.tsx` wraps the right-hand surface; `ui/ultra-chatbot-agent-version-footer.tsx` adds version navigation, restore, and latest reset; and `ui/ultra-chatbot-agent-diff-view.tsx` adds a lightweight diff preview for text artifact history.
