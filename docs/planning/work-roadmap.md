...
- Seven-day cleanup for visitor-created private demo data
  ...
  - Discussion: User confirmed on 2026-06-03 that "all persistent content" means user-created persistent demo data and that the retention window should be unified to seven days. Current implementation does not yet match that target: customer-memory and persistent-agent cleanup use three days, site-usage event cleanup uses thirty days, Ultra Chatbot Agent database rows still need scheduled cleanup, and Redis plus sandbox-backed surfaces still need an explicit cleanup story. Ultra Blob uploads now have a Blob SDK-backed 7-day cleanup path. Support waitlist entries remain a separate operating-data classification unless the product boundary later treats them as Private Demo Data.
