...
The remaining launch caveats are narrow: visitor identities are still resettable by clearing cookies, and public docs routes should stay free of private deployment notes.
...
- Customer Memory thread listing, loading, memory reads, memory mutations, and access-count updates filter by `customerId` plus `visitorId` (`apps/web/features/customer-memory-agent/server/thread-store.ts:159`, `apps/web/features/customer-memory-agent/server/thread-store.ts:276`, `apps/web/features/customer-memory-agent/server/memory-store.ts`).
...
### F1 - Low - Public docs MCP routes are read-only but expose repository docs
...
1. Keep public docs free of secrets and private deployment details.

## Current Readiness Call

For a friendly personal portfolio with low traffic, main chat spend is reasonably controlled and code-owned database, event, Blob, and tagged Sandbox resource retention now match the 7-day demo policy. For a public link that could receive automated traffic, cookie-reset limitations are still real enough to track, while the remaining listed item is mainly public-doc hygiene.
