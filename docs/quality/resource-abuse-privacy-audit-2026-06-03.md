...
The current repository is not ready for a public, unknown-traffic launch without a few focused fixes. The highest-risk remaining gap is that the "delete all DB and Blob content after 7 days" target is not implemented across all private demo stores.
...
- Ultra file uploads now require Visitor Owner routing plus `chatId`, write under `ultra-chatbot-agent/{visitorId}/{yyyy-mm-dd}/{chatId}/{uuid}-{filename}`, enforce a 20-file or 50MB per visitor/day Blob-list quota, delete Blob objects for per-chat and clear-history deletes, and run a 7-day Blob cleanup cron (`apps/web/app/api/demos/ultra-chatbot-agent/files/upload/route.ts`, `apps/web/features/ultra-chatbot-agent/server/upload.ts`, `apps/web/features/ultra-chatbot-agent/server/blob-storage.ts`, `apps/web/features/ultra-chatbot-agent/server/history.ts`, `apps/web/app/api/cron/ultra-chatbot-agent-blob-cleanup/route.ts`, `vercel.json`).
...
### F1 - Medium - 7-day DB and Blob retention target is not implemented across stores
...
- Ultra database tables have cascade paths from chats to messages/documents/suggestions/streams, but there is no scheduled Ultra database cleanup route yet; the current Ultra cron covers uploaded Blob objects only (`packages/database/src/schemas/ultra-chatbot-agent.ts:78`, `packages/database/src/schemas/ultra-chatbot-agent.ts:126`, `packages/database/src/schemas/ultra-chatbot-agent.ts:153`, `apps/web/app/api/cron/ultra-chatbot-agent-blob-cleanup/route.ts`).
- Site usage events retain for 30 days (`apps/web/features/site-usage-gate/server/cleanup.ts:1`).

Impact:

The current implementation does not meet the stated "all database and Blob content deleted after 7 days" target. Some user-private Customer Memory event text can persist, Ultra chat data has no automatic expiration, and site usage telemetry keeps longer-lived visitor records. Ultra uploaded Blob objects now have a 7-day cleanup path.

Recommendation:

Define a single `Demo Data Retention Window` constant or clearly documented per-store policy. For the user's current target, add a 7-day cleanup for Ultra chats, messages, documents, suggestions, streams, and votes; decide whether Customer Memory/Persistent Agent should remain stricter at 3 days or align to 7 days; delete Customer Memory events that contain private text; classify site usage/waitlist separately from private demo content.
...
1. Implement the 7-day cleanup story for Ultra DB rows and Customer Memory events.
2. Decide whether Customer Memory/Persistent Agent should stay stricter at 3 days or align to the unified 7-day Demo Data Retention Window.
3. Add production `Secure` cookies and owner predicates to Customer Memory mutations.

## Current Readiness Call

For a friendly personal portfolio with low traffic, main chat spend is reasonably controlled and Ultra upload abuse now has a demo-stage cap plus cleanup path. For a public link that could receive automated traffic, the remaining retention and visitor-cookie limitations are still real enough to fix before launch.
