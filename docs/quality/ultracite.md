...
- Biome GritQL plugins reject hardcoded Tailwind palette utilities and CSS color literals in app-owned UI and shared base components. Define semantic status tokens in `packages/ui/src/styles/globals.css`; imported `packages/ui/src/components/ai-elements/**` stays excluded so upstream refreshes remain low-conflict. Run `pnpm test:biome-colors` for the focused rule contract.
