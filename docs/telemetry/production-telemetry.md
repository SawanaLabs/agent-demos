...
All other ready demos share the metered host route error adapter. One terminal thrown failure or final 5xx response records a bounded provider or storage event based on the catalog action; expected 4xx/429 responses and successful routes do not log. `send_message`, `evaluate`, and `generate_object` classify as provider operations; `compact_context` and `edit_message` classify as storage mutations. Routes with a deeper provider-neutral observer, currently Image Workflow, opt out of this final-route fallback so one failure cannot be recorded twice. Realtime credential setup is excluded from Product Analytics while its unexpected route failure remains observable.
