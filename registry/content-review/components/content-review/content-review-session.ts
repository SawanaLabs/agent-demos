...
export interface DisplayReviewThreadEntry extends ReviewThreadEntry {
  isActive: boolean;
  liveResult: DeepPartial<ContentReviewResult> | undefined;
...
export function buildPendingRecord(recordId: string): ContentReviewRecord {
  return {
    errorMessage: null,
    id: recordId,
    recordedAt: null,
...
export function toDisplayReviewThreadEntries(
  ...
  isLoading: boolean,
  liveObject: DeepPartial<ContentReviewResult> | undefined
...
export function readRecordIdFromResponse(response: Response) {
  return response.headers.get(contentReviewRecordIdHeader);
}
